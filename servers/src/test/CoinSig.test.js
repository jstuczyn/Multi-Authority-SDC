...
import { getCoin, getRandomCoinId, hashToBIG, hashG2ElemToBIG } from '../auxiliary';
...
describe('CoinSig Scheme', () => {
  ...
  describe('Sign', () => {
    it('For signature(sig1, sig2), sig2 = (x0 + x1*a1 + x2*a2 + ...) * sig1', () => {
      ...
      const [dummyCoin, dummyCoin_id] = getCoin(coin_pk, 42);
  ...
  describe('Aggregate', () => {
    ...
    it('Returns null if one of signatures is invalid (different h)', () => {
      ...
      const [dummyCoin1, dummyCoin_id1] = getCoin(coin_pk1, 42);
      const [dummyCoin2, dummyCoin_id2] = getCoin(coin_pk2, 43);
  ...
  describe('Aggregate Verification', () => {
    ...
    describe('Aggregate Verification', () => {
      ...
      it('Doesn\'t work when one of three signatures is on different coin (but for some reason has same ID)', () => {
        ...
        const [anotherCoin, anotherCoin_id] = getCoin(another_coin_pk, 43);
        anotherCoin.ID = new G.ctx.ECP();
  ...
  describe('Blind Signature', () => {
    it('Can produce a blind signature and successfuly verify it after decryption', () => {
      const params = CoinSig.setup();
      const [G, o, g1, g2, e] = params;

      const [ElGamalSK, ElGamalPK] = ElGamal.keygen(params);
      const [sa_sk, sa_pk] = CoinSig.keygen(params);

      const [g, X0, X1, X2, X3, X4] = sa_pk;

      const [coin_sk, coin_pk] = Coin.keygen(params);
      const id = getRandomCoinId();
      const coin = new Coin(coin_pk, id, 42);

      // this ensures the appropriate fields are encrypted plus the coin will always undergo this processs before being sent
      // also simulates being sent
      const signingCoin = coin.prepareCoinForSigning(ElGamalPK, params, id, coin_sk);
      const recreatedCoin = Coin.fromSigningCoin(signingCoin);

      const [h, enc_sig] = CoinSig.mixedSignCoin(params, sa_sk, recreatedCoin, ElGamalPK);

      const sig = ElGamal.decrypt(params, ElGamalSK, enc_sig);

      const pkX = G.ctx.PAIR.G2mul(X3, coin_sk);

      assert.isTrue(CoinSig.verifyMixedBlindSign(params, sa_pk, coin, [h, sig], id, pkX));
    });
  });
});
